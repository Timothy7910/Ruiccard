...
async function init() {
  ...
  for (const key of Object.keys(config.assets)) config.assets[key] = new URL(config.assets[key], requestedConfig).href;
  ...
  try {
    renderer = new THREE.WebGLRenderer({
      antialias: true,
      alpha: new URLSearchParams(location.search).has('embed'),
  ...
  renderer.setClearColor(config.appearance?.background || "#fafafa", new URLSearchParams(location.search).has('embed') ? 0 : 1);
  ...
  const materials = {
    ...
    web_gold: new THREE.MeshBasicMaterial({ color: config.appearance?.edgeColor || "#94ced6" }),
  };
