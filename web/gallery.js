import {locale,languageReady,translate} from './i18n.js';
...
const group=c=>c.category||(['鳴潮','鸣潮'].includes(c.series)?'wuthering-waves':'other');
...
async function select(i,{historyMode='push'}={}){
 ...
 const viewer=new URL('./viewer.html',location.href);viewer.searchParams.set('config',card.config);viewer.searchParams.set('lang',locale);$('full-view').href=viewer.href;viewer.searchParams.set('embed','1');frame.title=card.title+'互動 3D 卡牌';frame.contentWindow.location.replace(viewer.href);
 document.title=`${card.title} — 靈境典藏`;
 const url=new URL(location.href);url.search='';url.searchParams.set('card',card.id);url.searchParams.set('lang',locale);if(historyMode==='push')history.pushState({},'',url);else if(historyMode==='replace')history.replaceState({},'',url);
...
window.addEventListener('keydown',e=>{if(document.querySelector('dialog[open]')||e.target.matches('input,select,textarea'))return;if(e.key==='ArrowRight'&&cards.length>1)select(index+1);if(e.key==='ArrowLeft'&&cards.length>1)select(index-1);});
frame.addEventListener('load',()=>{frame.contentDocument?.addEventListener('pointerup',()=>setTimeout(sync,50));});
async function init(){try{await languageReady;const res=await fetch(locale==='zh-Hans'?'./collection.zh-Hans.json':'./collection.json');if(!res.ok)throw Error('missing collection');cards=(await res.json()).cards;if(!Array.isArray(cards)||!cards.length)throw Error('empty collection');$('total').textContent=pad(cards.length);$('prev').disabled=$('next').disabled=cards.length<2;
